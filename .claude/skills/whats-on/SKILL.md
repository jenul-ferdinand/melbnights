---
name: whats-on
description: Find what's on in Melbourne for a day or weekend, with an optional vibe (rave, cheap, uni, gig, bar, day, festival). Use for "/whats-on", "what's on tonight", "anything on friday", "where should we go this weekend", "any festivals coming up".
---
...
4. `scripts/ra.sh <date> <date>` for club/rave listings. Fetch other `sources.md` URLs only if the vibe calls for them (gig -> Beat, bar -> Broadsheet, cheap/uni -> Eventbrite free + WebSearch, day -> What's On Melbourne, festival -> `annual.md` then Moshtix). "day" means start before 18:00.
